...
func main() {
	...
	gaugeObserverBool, err := meter.Float64ObservableGauge("observer-boolean-collector")
	if err != nil {
		log.Fatalf("failed to initialize instrument: %v", err)
	}
	_, err = meter.RegisterCallback(func(ctx context.Context, o metric.Observer) error {
		v := of.get()
		var strBoolVal string
		if v > 15.0 {
			strBoolVal = "true"
		} else {
			strBoolVal = "false"
		}
		// These attributes make sure that googlecloudexporter knows to export this as a BOOL type.
		gaugelabels := []attribute.KeyValue{
			attribute.Key("gcp.cloud_monitoring.custom.type").String("BOOL"),
			attribute.Key("gcp.cloud_monitoring.custom.value").String(strBoolVal),
		}
		o.ObserveFloat64(gaugeObserverBool, v, metric.WithAttributes(gaugelabels...))
		return nil
	}, gaugeObserverBool)
	if err != nil {
		log.Fatalf("failed to register callback: %v", err)
